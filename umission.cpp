...
bool UMission::trappedims(int & state) // gullutine port -> ramp up, tippe ned ramp up trappe ned stop 
{
	...
	case 20: //Through Gate
		...
		snprintf(lines[7], MAX_LEN, "vel=0.2,acc=2:time=2,dist=0.2");
		snprintf(lines[8], MAX_LEN, "vel=0.3,acc=2,edger=0.0:xl>8");
	...
	case 30: //Through Gate
		...
		snprintf(lines[1], MAX_LEN, "vel=0.2,acc=5,edger=1.0,white=1:dist=1.95");
		snprintf(lines[2], MAX_LEN, "vel=0,acc=100:time=2");
		snprintf(lines[3], MAX_LEN, "vel=0.1,acc=1,edger=1.0,white=1:xl>5");
		snprintf(lines[4], MAX_LEN, "vel=0.2,acc=100:time=2,dist=0.1");
		snprintf(lines[5], MAX_LEN, "vel=0.3,acc=1,edgel=1.0,white=1:xl>16");
		snprintf(lines[6], MAX_LEN, "vel=0,acc=100:time=0.5");
