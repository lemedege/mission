...
bool UMission::trappedims(int & state) // gullutine port -> ramp up, tippe ned ramp up trappe ned stop 
{
	...
	case 10: //Through Gate
		...
		snprintf(lines[5], MAX_LEN, "vel=0.2,acc=2.0,edgel=0,white=1:dist=0.40");
		snprintf(lines[6], MAX_LEN, "tr=0.0,vel=0.3,acc=2:turn=-90");
		snprintf(lines[7], MAX_LEN, "vel=0.3,acc=2.0,edgel=0,white=1:dist=0.4");
		snprintf(lines[8], MAX_LEN, "vel=0.2,acc=2.0,edgel=0,white=1:xl>5");
	...
	case 20: //Through Gate
		...
		snprintf(lines[9], MAX_LEN, "vel=0,acc=100:time=0.4");
		snprintf(lines[10], MAX_LEN, "event=1:time=1.1");
				missionSendAndRun(lineList, 11);
		state++;
		break;
	...
	case 30: //Through Gate
		snprintf(lines[0], MAX_LEN, "tr=0,vel=0.5,acc=2:turn=-90");
		snprintf(lines[1], MAX_LEN, "vel=0.2,acc=5,edger=1.0,white=1:dist=1.8");
		snprintf(lines[2], MAX_LEN, "vel=-0.15,acc=100:time=7");
		snprintf(lines[3], MAX_LEN, "vel=0.2,acc=5,edger=1.0,white=1:dist=0.7");
		snprintf(lines[4], MAX_LEN, "vel=0.2,acc=1,edgel=1.0,white=1:xl>10");
...
bool UMission::trappetogoal(int & state) // from trappe end to goal
{
	...
	case 0: //Through Gate
		snprintf(lines[0], MAX_LEN, "tr=0.0,vel=0.2,acc=1:turn=-90");
		snprintf(lines[1], MAX_LEN, "vel=0.3,acc=1,edger=1.0,white=1:dist=10");
		snprintf(lines[2], MAX_LEN, "event=1:time=1.1");
				missionSendAndRun(lineList, 3);
		state++;
		break;
...
bool UMission::trappetoreg(int & state) // from trappe end to regbot (aka center)
{
	...
	case 0: //Through Gate
		...
		snprintf(lines[1], MAX_LEN, "vel=0.0,acc=100.0:time=0.1");
		snprintf(lines[2], MAX_LEN, "vel=0.3,acc=2,edgel=0.0, white=1 : dist=0.25");
		snprintf(lines[3], MAX_LEN, "vel=0.2,acc=2.0, edger=0.0, white=1:xl>16");
...
bool UMission::regtogoal(int & state) // from regbot to goal
{
	...
	case 0: //Through Gate
        ...
        snprintf(lines[4], MAX_LEN, "vel=0.35,acc=5, edgel=0.0, white=1:xl>12");
		snprintf(lines[5], MAX_LEN, "tr=0.0,vel=0.3,acc=2:turn=-90");
		snprintf(lines[6], MAX_LEN, "vel=0.35,acc=5, edger=1.0, white=1:dist=2.6");
		snprintf(lines[7], MAX_LEN, "vel=0.35,acc=5, edgel=0.0, white=1:xl>10");
...
bool UMission::tunel(int & state) // trough tunnel closing doors a
{
	...
	case 20: //Through Gate
		...
		snprintf(lines[3], MAX_LEN, "vel=0,acc=3:time=0.3");
                ...
                snprintf(lines[6], MAX_LEN, "tr=0.1,vel=0.3,acc=3:turn=-90");
                snprintf(lines[7], MAX_LEN, "vel=0.3,acc=5:dist=0.95");
				snprintf(lines[8], MAX_LEN, "tr=0.2,vel=0.3,acc=3:turn=-90");
				snprintf(lines[9], MAX_LEN, "vel=0.3,acc=5,white=1:dist=0.3");
				snprintf(lines[10], MAX_LEN, "event=1:time=1.1");
				missionSendAndRun(lineList, 11);
        state++;
        break;
	...
	case 30: //Through Gate
				...
				snprintf(lines[0], MAX_LEN, "vel=0.45,acc=5,white=1:xl>18");
				snprintf(lines[1], MAX_LEN, "tr=0.0,vel=0.3,acc=3:turn=-90");
				snprintf(lines[2], MAX_LEN, "vel=0.1,acc=3,edgel=0.0,white=1:time=7.0");
				snprintf(lines[3], MAX_LEN, "vel=-0.1,acc=3:dist=0.1");
                snprintf(lines[4], MAX_LEN, "vel=-0.1,acc=3:dist=0.1");
                snprintf(lines[5], MAX_LEN, "tr=0.05,vel=0.3,acc=3:turn=90");
                snprintf(lines[6], MAX_LEN, "vel=0.3,acc=3:dist=0.50");
                snprintf(lines[7], MAX_LEN, "tr=0.05,vel=0.3,acc=3:turn=-90");
                snprintf(lines[8], MAX_LEN, "vel=0.3,acc=3:dist=0.95");
	...
	case 40: //Through Gate
				snprintf(lines[0], MAX_LEN, "vel=0.3,acc=5,white=1:xl>18");
				snprintf(lines[1], MAX_LEN, "tr=0.0,vel=0.3,acc=3:turn=-90");
				snprintf(lines[2], MAX_LEN, "vel=0.25,acc=3,edgel=0.0,white=1:time=7.0");
				snprintf(lines[3], MAX_LEN, "vel=-0.1,acc=3:dist=0.1");
				snprintf(lines[4], MAX_LEN, "vel=-0.1,acc=3:dist=0.1");
                snprintf(lines[5], MAX_LEN, "tr=0.0,vel=0.3,acc=3:turn=180");
                snprintf(lines[6], MAX_LEN, "vel=-0.35,acc=30:time=4");
				snprintf(lines[7], MAX_LEN, "vel=0.0, acc=100.0, edgel=0.0, white=1:time=0.5");
                snprintf(lines[8], MAX_LEN, "event=1:time=1.1");
				missionSendAndRun(lineList, 9);
        state++;
        break;
...
switch (state)
{
	...
	snprintf(lines[4], MAX_LEN, "vel=0.0, acc=100.0:time=0.2");
	snprintf(lines[5], MAX_LEN, "event=1:time=1.1");
    missionSendAndRun(lineList, 6);
    state++;
    break;
...
switch (state)
{
    ...
    snprintf(lines[0], MAX_LEN, "vel=0.5,acc=4,edger=0.0,white=1:dist=0.3");
                snprintf(lines[1], MAX_LEN, "vel=1.5,acc=4,edgel=0.0,white=1:dist=0.3");
				...
				snprintf(lines[5], MAX_LEN, "vel=0,acc=1000:time=2");
				snprintf(lines[6], MAX_LEN, "event=1:time=1.1");
    missionSendAndRun(lineList, 7);
    state++;
    break;
...
{
    case 0: //race track 
				snprintf(lines[0], MAX_LEN, "vel=0,acc=1000:time=1");
				snprintf(lines[1], MAX_LEN, "tr=0.05,vel=0.2,acc=3:turn=-90");
                snprintf(lines[2], MAX_LEN, "vel=0.4,acc=4,edger=0.0,white=1:dist=2.25");
                snprintf(lines[3], MAX_LEN, "vel=0,acc=1000:time=1");
                snprintf(lines[4], MAX_LEN, "tr=0.0,vel=0.25,acc=3:turn=180");
...
{
    case 0:
                snprintf(lines[0], MAX_LEN, "vel=0.2,acc=4,edger=0.0,white=1:xl>4");
				snprintf(lines[1], MAX_LEN, "tr=0.05,vel=0.3,acc=3:turn=90");
				snprintf(lines[2], MAX_LEN, "vel=0.3,acc=4,edger=0.0,white=1:dist=3.55");
				snprintf(lines[3], MAX_LEN, "vel=0.2,acc=2,edger=0.0,white=1:xl>4");
				snprintf(lines[4], MAX_LEN, "event=1:time=1.1");
                missionSendAndRun(lineList, 5);
                state++;
                break;
	...
	case 20: 
                ...
                snprintf(lines[0], MAX_LEN, "vel=0.0,acc=100.0:time=0.1");
                snprintf(lines[1], MAX_LEN, "vel=0.0,acc=100.0:ir2<0.21");
                snprintf(lines[2], MAX_LEN, "vel=0.0,acc=5:ir2>0.4");
                snprintf(lines[3], MAX_LEN, "vel=0.0,acc=5:time=0.1");
				snprintf(lines[4], MAX_LEN, "vel=0.5,acc=5, edgel=0.0, white=1:xl>6");
				snprintf(lines[5], MAX_LEN, "vel=0.0,acc=100.0:time=0.1");
				snprintf(lines[6], MAX_LEN, "tr=0.0,vel=0.3,acc=3:turn=90");
				snprintf(lines[7], MAX_LEN, "vel=0.4,acc=2,edgel=0.0, white=1 :dist=3");
				snprintf(lines[8], MAX_LEN, "event=1:time=1.1");
                missionSendAndRun(lineList, 9);
                state++;
                break;
